  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
          ...
          >
            ...
            <ul className="list-none mt-4">
              {footerLinks.links.map((link, index) => (
                <li
                  key={link.name}
                  className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== footerLinks.links.length -1 ? 'mb-4' : 'mb-0'} `}
                >
                  {link.name}
                </li>
              ))}
    ...
    <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3E45]">
        <p className="font-poppins font-medium text-[18px] leading-[27px] text-white">
            2021 hookbank . all right reserved
        </p>
